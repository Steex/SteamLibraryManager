...
namespace SteamLibraryManager
{
	...
	internal class SteamKeyValueReader : StreamReader
	{
	...
	public class SteamKeyValue
	{
		...
		public SteamKeyValue(string name = null, string value = null)
		{
			this.Name = name;
			this.Value = value;
		...
		/// </summary>
		public readonly static SteamKeyValue Invalid = new SteamKeyValue();
		...
		/// </summary>
		public List<SteamKeyValue> Children { get; private set; }
		...
		/// </remarks>
		public static SteamKeyValue LoadAsText(string path)
		{
		...
		{
			this.Children = new List<SteamKeyValue>();

			new SteamKeyValueReader(this, input);
		...
		private void RecursiveSaveTextToFile(Stream stream, int indentLevel = 0)
		{
			...
			foreach (SteamKeyValue child in Children)
			{
				if (child.Value == null)
				{
